...
SELECT 
MONTH(sale_date) AS month ,
ROUND(AVG(x.per_day_revenue) ,4) AS avg_revenue_2023
...
ORDER BY avg_revenue_2023 DESC ;

--2022
SELECT 
MONTH(sale_date) AS month ,
ROUND(AVG(x.per_day_revenue) ,4) AS avg_revenue_2022
FROM ( SELECT  sale_date ,  SUM(total_sale)AS per_day_revenue 
		FROM retail_sales 
		WHERE YEAR(sale_date)=2022
		GROUP BY sale_date 
...
GROUP BY MONTH(sale_date) 
ORDER BY avg_revenue_2022 DESC ;
...
SELECT COUNT(DISTINCT x.customer_id) AS repeat_customers
FROM (
		SELECT customer_id, YEAR(sale_date) AS year,  COUNT(customer_id) AS purchase_count 
		FROM retail_sales
		GROUP BY YEAR(sale_date) , customer_id )x
...
SELECT category , YEAR(sale_date) AS year, SUM(quantity) AS total_units , SUM(total_sale) AS total_revenue 
...
SELECT YEAR(sale_date) AS year , gender ,  SUM(total_sale) AS net_revenue  
FROM retail_sales
GROUP BY YEAR(sale_date) , gender; 
...
--Calculate the profit_margin made on each category  
SELECT category , (SUM(net_profit)/SUM(total_sale))*100 AS categorical_profit_margin 
FROM retail_sales
GROUP BY category ;

--Average Selling Price (ASP) of each product category . 
SELECT category , MONTH(sale_date) , AVG(total_sale) AS asp
FROM retail_sales
GROUP BY category , MONTH(sale_date)
